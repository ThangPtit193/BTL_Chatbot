...
class Document:
    ...
    def __init__(
        ...
        :param id_hash_keys: Generate the document id from a custom list of strings that refers to the documents attributes.
        ...
        if id_hash_keys is not None:
            if not set(id_hash_keys) <= set(allowed_hash_key_attributes):  # type: ignore
                raise ValueError(
                    ...
                    f"list of Document's attribute names that the id should be based on "
                    f"(e.g. {allowed_hash_key_attributes}). "
                )
    ...
    def __str__(self):
        # In some cases, self.content is None (therefore not subscript-able)
...
class Answer:
    ...
    def __str__(self):
        # self.context might be None (therefore not subscript-able)
        if not self.context:
    ...
    def from_dict(cls, doc: dict):
        return _pydantic_dataclass_from_dict(doc=doc, pydantic_dataclass_type=cls)
...
class Label:
    ...
    def __init__(
        ...
        :param id: Unique ID used within the DocumentStore. If not supplied, an uuid will be generated automatically.
...
class MultiLabel:
    ...
    def __init__(self, labels: List[Label], drop_negative_labels=False, drop_no_answers=False):
        ...
        if drop_negative_labels:
            labels = [label for label in labels if is_positive_label(label)]
        ...
        self.no_answer = False not in [label.no_answer for label in self.labels]
        ...
        if self.no_answer:
            ...
            answered = [label.answer for label in self.labels if not label.no_answer and label.answer is not None]
        ...
        self.document_ids = [label.document.id for label in self.labels if not label.no_answer]
        self.contexts = [str(label.document.content) for label in self.labels if not label.no_answer]

    def _aggregate_labels(self, key, must_be_single_value=True) -> List[Any]:
        if any(isinstance(getattr(label, key), dict) for label in self.labels):
            # dict is not hashable, so we collect unique filters via looping through all labels
            unique_values = []
            for label in self.labels:
                if label.filters not in unique_values:
                    unique_values.append(label.filters)
        else:
            unique_values = list({getattr(label, key) for label in self.labels})
...
def _pydantic_dataclass_from_dict(doc: dict, pydantic_dataclass_type) -> Any:
    """
    ...
    :param doc: Dict containing all attributes and values for the dataclass.
    :param pydantic_dataclass_type: The class of the dataclass that should be constructed (e.g. Document)
    """
    base_model = pydantic_dataclass_type.__pydantic_model__.parse_obj(doc)
...
class EvaluationResult:
    ...
    def _build_answer_metrics_df(
        ...
        - sas if a SAS model has bins provided during pipeline.eval() (How semantically similar is the prediction to the gold answers?)
        """
    ...
    def _build_document_metrics_df(
        ...
        """
        Builds a dataframe containing document metrics (columns) per a pair of query and gold document ids (index).
        ...
        for multilabel_id in documents["multilabel_id"].unique():
            ...
            num_relevant_docs = len(set(gold_ids + relevance_criterion_ids))
            num_retrieved_relevant_docs = query_df[relevance_criterion_col].values.sum()
            rank_retrieved_relevant_docs = query_df[query_df[relevance_criterion_col] == 1]["rank"].values
            avp_retrieved_relevant_docs = [
                query_df[relevance_criterion_col].values[: int(rank)].sum() / rank for rank in
                rank_retrieved_relevant_docs
            ]

            avg_precision = np.sum(avp_retrieved_relevant_docs) / num_relevant_docs if num_relevant_docs > 0 else 0.0
            recall_multi_hit = num_retrieved_relevant_docs / num_relevant_docs if num_relevant_docs > 0 else 1.0
            recall_single_hit = min(num_retrieved_relevant_docs, 1) if num_relevant_docs > 0 else 1.0
            precision = num_retrieved_relevant_docs / retrieved if retrieved > 0 else 0.0
            rr = 1.0 / rank_retrieved_relevant_docs.min() if len(rank_retrieved_relevant_docs) > 0 else 0.0
            dcg = (
                np.sum([1.0 / np.log2(rank + 1) for rank in rank_retrieved_relevant_docs])
                if len(rank_retrieved_relevant_docs) > 0
                else 0.0
            )
            idcg = (
                np.sum([1.0 / np.log2(rank + 1) for rank in
                        range(1, num_relevant_docs + 1)]) if num_relevant_docs > 0 else 1.0
            )
            ndcg = dcg / idcg
    ...
    def save(self, out_dir: Union[str, Path], **to_csv_kwargs):
        ...
        for node_name, df in self.node_results.items():
            ...
            default_to_csv_kwargs = {
                ...
                # avoids problems with \r chars in texts by enclosing all string values in quotes
            }
