...
from meteor.nodes.evaluator.evaluator import semantic_answer_similarity
from meteor.modelling.evaluation.squad import compute_f1 as calculate_f1_str
from meteor.modelling.evaluation.squad import compute_exact as calculate_em_str
from meteor.pipelines.config import (
...
class Pipeline:
    ...
    Under the hood, a Pipeline is represented as a directed acyclic graph of component nodes.
    You can use it for custom query flows with the option to branch queries (for example, extractive question answering
    and keyword match query), merge candidate documents for a Reader from multiple Retrievers,
    or re-ranking of candidate documents.
    """
    ...
    def add_node(self, component: BaseComponent, name: str, inputs: List[str]):
        ...
        # The document store is already being used by the retriever.
    ...
    def eval_beir(
        ...
        try:
            ...
            raise MeteorError("beir is not installed. Please run `pip install beir`...") from e
        ...
        if document_store is not None:
            if document_store.get_document_count() > 0:
                raise MeteorError(f"Index '{document_store.index}' is not empty. Please provide an empty index.")
        ...
        meteor_retriever = _MeteorBeirRetrieverAdapter(
            index_pipeline=index_pipeline,
            query_pipeline=query_pipeline,
            index_params=index_params,
            query_params=query_params,
        )
        retriever = EvaluateRetrieval(meteor_retriever, k_values=top_k_values)
    ...
    def execute_eval_run(
        ...
        :param experiment_tracking_uri: The uri of the experiment tracking server to be used. Must be specified if experiment_tracking_tool is set.
                                        Note, that artifact logging (e.g. Pipeline YAML or evaluation result CSVs)
        ...
        if experiment_tracking_tool is not None:
            ...
            if experiment_tracking_uri is None:
                raise MeteorError(f"experiment_tracking_uri must be specified if experiment_tracking_tool is set.")
        ...
        try:
            ...
            if document_store is None:
                raise MeteorError(f"Document store not found. Please provide pipelines with proper document store.")
    ...
    def eval(
        ...
        - `custom_document_id_field` param to select a custom document ID from document's metadata for ID matching (only affects 'document_id' scopes)
    ...
    def eval_batch(
        ...
        :param labels: The labels to evaluate on.
        :param documents: List of Document that the first node in the pipeline gets as input per multilabel. You can use it to evaluate a pipeline that consists of a reader without a retriever.
        :param params: Dictionary of parameters to be dispatched to the nodes.
                    To pass a parameter to all nodes, just use: {"top_k":10}.
                    To pass a parameter to targeted nodes, you can type:
                    {"Retriever": {"top_k": 10}, "Reader": {"top_k": 3, "debug": True}}
            ...
            for field_name in ["answers", "answers_isolated"]:
                ...
                if answers is not None:
                    if i < len(answers) and isinstance(answers[i], list):
                        # answers_isolated refers to only one relevant document and thus only a list of
                        # answers refers to multiple relevant documents and thus multiple lists of answers
                        answers = answers[i]
                    if len(answers) == 0:
    ...
    def load_from_yaml(
        ...
        Here's a sample configuration:
            ...
            |      type: BM25Retriever
            |      params:
            |        document_stores: MyDocumentStore    # params can reference other components defined in the YAML
    ...
    def get_config(self, return_defaults: bool = False) -> dict:
        ...
        for node_name, node_attributes in self.graph.nodes.items():
            ...
            for predecessor in self.graph.predecessors(node_name):
                ...
                # if there are multiple outputs, we're not coming from a root node we have to specify the stream
...
class _MeteorBeirRetrieverAdapter:
    ...
    def search(self, corpus: Dict[str, Dict[str, str]], queries: Dict[str, str], top_k: int,
               score_function: Optional[str] = None, **kwargs
