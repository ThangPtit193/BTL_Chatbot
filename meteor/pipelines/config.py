...
def validate_yaml(
    ...
    :param path: path to the YAML file to validate the configuration to validate
    ...
    logging.debug("'%s' contains valid Meteor pipelines.", path)
...
def validate_schema(pipeline_config: Dict, strict_version_check: bool = False, extras: Optional[str] = None) -> None:
    ...
    while True:
        try:
            ...
            error_path = [
                i
                for i in list(validation.schema_path)[:-1]
