...
from meteor.schema import Document, Label, MultiLabel
from meteor.nodes.base import BaseComponent
from meteor.errors import DuplicateDocumentError, DocumentStoreError
...
class BaseDocumentStore(BaseComponent):
    ...
    def get_all_labels_aggregated(
        ...
        This aggregation step helps, for example, if you collected multiple possible answers for one question you
        ...
        If the questions are being asked  a single document (i.e. SQuAD style), you should set open_domain=False to aggregate by question and document.
        If the questions are being asked  your full collection of documents, you should set open_domain=True to aggregate just by question.
        If the questions are being asked a sub-slice of your document set (e.g. product review use cases),
    ...
    def add_eval_data(
        ...
        If a jsonl file and a batch_size is passed to the function, documents are loaded batch-wise
        from disk and also indexed batch-wise to the DocumentStore in order to prevent out of memory errors.
        ...
        if file_path.suffix == ".json":
            if batch_size is None:
                docs, labels = eval_data_from_json(
                    filename, max_docs=max_docs, open_domain=open_domain
                )
...
class KeywordDocumentStore(BaseDocumentStore):
    ...
    def query(
        ...
        :param all_terms_must_match: Whether all terms of the query must match the document.
                                     ...
                                     (i.e. the AND operator is being used implicitly between query terms:
    ...
    def query_batch(
        ...
        This method lets you find relevant documents for a single query string (output: List of Documents), or a list of
        query strings (output: List of Lists of Documents).
