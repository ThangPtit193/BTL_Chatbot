...
from meteor import __version__ as meteor_version
from meteor.errors import PipelineSchemaError
from meteor.nodes.base import BaseComponent

logger = logging.getLogger(__name__)

JSON_SCHEMAS_PATH = Path(__file__).parent.parent.parent / "meteor" / "json-schemas"
SCHEMA_URL = "https://raw.githubusercontent.com/knowledge-retrieval/meteor-json-schema/main/json-schema/"

# Allows accessory classes (like enums and helpers) to be registered as valid input for
# custom node's init parameters. For now, we disable this feature, but flipping these variables
    ...
    component_schema = {
        component_name: {
            ...
            "properties": {
                ...
                "type": {
                    "title": "Type",
                    "description": "Meteor Class name for the component.",
                    "type": "string",
                    "const": f"{node_name}",
                },
...
def get_json_schema(filename: str, version: str, modules: List[str] = ["meteor.document_stores", "meteor.nodes"]):
    """
    Generate JSON schema for Meteor pipelines.
    """
    ...
    pipeline_schema = {
        ...
        "title": "Meteor Pipeline",
        "description": "Meteor Pipeline YAML file describing the nodes of the pipelines.",
        "type": "object",
        "properties": {
            "version": {
                "title": "Version",
                "description": "Version of the Meteor Pipeline file.",
                "type": "string",
                "const": version,
            },
            "extras": {
            ...
            "components": {
                ...
                # To allow for custom components in IDEs - will be set to False at validation time.
            },
            "pipelines": {
                "title": "Pipelines",
...
def update_json_schema(destination_path: Path = JSON_SCHEMAS_PATH, main_only: bool = False):
    ...
    filename = f"meteor-pipeline-main.schema.json"
    ...
    if not main_only and "rc" not in meteor_version:

        # Create/update the specific version file too
        filename = f"meteor-pipeline-{meteor_version}.schema.json"
        with open(destination_path / filename, "w") as json_file:
            json.dump(get_json_schema(filename=filename, version=meteor_version), json_file, indent=2)

        # Update the index
        index_name = "meteor-pipeline.schema.json"
        with open(destination_path / index_name, "r") as json_file:
            index = json.load(json_file)
            new_entry = {
                "allOf": [
                    {"properties": {"version": {"const": meteor_version}}},
                    {"$ref": f"{SCHEMA_URL}meteor-pipeline-{meteor_version}.schema.json"},
                ]
            }
