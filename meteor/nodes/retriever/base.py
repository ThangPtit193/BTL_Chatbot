...
class BaseGraphRetriever(BaseComponent):
    """
    Base class for knowledge graph retrievers.
    """
        ...
        if isinstance(queries, str):
            ...
            for doc in documents:
                if not isinstance(doc, Document):
                    raise MeteorError(f"doc was of type {type(doc)}, but expected a Document.")
                document_ids.append(doc.id)
            logger.debug("Retrieved documents with IDs: %s", document_ids)
        else:
            for doc_list in documents:
                if not isinstance(doc_list, list):
                    raise MeteorError(f"doc_list was of type {type(doc_list)}, but expected a list of Documents.")
