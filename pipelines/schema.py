...
class Document:
    ...
    def __init__(
            ...
            embedding: Optional[np.ndarray] = None,
            id_hash_keys: Optional[List[str]] = None,
            index: Optional[str] = "default"

    ):
        """
        One of the core data classes in PIPELINES. It's used to represent documents / passages in a standardized
        way within PIPELINES.
        Documents are stored in DocumentStores, are returned by Retrievers, are the input for Readers and are
        used in many other places that manipulate or interact with document-level data.
        ...
        It's particularly helpful for handling of duplicates and referencing documents in other objects
        (e.g. Labels)

        There's an easy option to convert from/to dicts via `from_dict()` and `to_dict`.

        :param content: Content of the document. For most cases, this will be text, but it can be a table or
                        image. Tabular data or images will be converted into text.
        :param content_type: One of "image", "table" or "image". PIPELINES components can use this to adjust
                             their handling of Documents and check compatibility.
        :param id: Unique ID for the document. If not supplied by the user, we'll generate one automatically by
                   creating a hash from the supplied text. This behaviour can be further adjusted by `
                   id_hash_keys`.
        ...
        :param meta: Meta fields for a document like name, url, or author in the form of a custom dict
                     (any keys and values allowed).
        :param embedding: Vector encoding of the text
        :param id_hash_keys: Generate the document id from a custom list of strings that refer to the documents
                             attributes.
                             If you want to ensure you don't have duplicate documents in your DocumentStore but
                             texts are not unique, you can modify the metadata and pass e.g. "meta" to this field
                             (e.g. ["content", "meta"]). In this case the id will be generated by using the
                             content and the defined metadata.
        :param index: Index of document. To improve searching performance, indexing is an essential to narrow
                      the document searched. In case of none index, index will be named `default`
                """
        if content is None:
            raise ValueError(
                f"Fail to create Document: `content` field is mandatory"
            )
        ...
        allowed_hash_key_attributes = [
            "content", "content_type", "score", "index"
        ]

        if id_hash_keys is not None:
            if not set(id_hash_keys) <= set(allowed_hash_key_attributes):
                raise ValueError(
                    f"You passed custom strings {id_hash_keys} to `id_hash_keys` which is deprecated."
                    f"Supply instead a list of Document's attribute names that the id should be based on "
    ...
    def _get_id(self, id_hash_keys: Optional[List[str]] = None) -> str:
        """
        ...
        if id_hash_keys is None:
            return DeepHash(str(self.content))[self.content]
        ...
        if final_hash_key == "":
            raise ValueError(
                f"Cannot create 'Document': `id_hash_keys` must contain at least one of ['content', 'meta']"
            )

        return DeepHash(str(final_hash_key))[final_hash_key]

    def to_dict(self, field_map: Dict) -> Dict:
        """
        Convert Document to dict. An optional field_map can be supplied to change the names of the keys in the
        resulting dict. This way you can work with standardized Document objects in PIPELINES, but adjust the format
        that they are serialized / stored in other places (e.g. elasticsearch)
        Example:
    ...
    @classmethod
    def from_dict(cls, dict: Dict[str, Any], field_map: Dict[str, Any], id_hash_keys=None):
        """
